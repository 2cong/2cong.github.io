...
- 가능하다면 주석을 달기 보다는 함수 이름에 정보를 담는 것이 더 나음
...
- 가능하다면 인수나 반환 값 자체를 명확하게 만드는 것이 좋음
- 🔥 Example
...
- 때로는 docs도 잡음이 됨
    ...
    → 수행하는 목적이 없으며 문서를 제공해야 한다는 욕심으로 작성한 docs
...
- 🔥 Example
    ...
    */
    ...
    → 위의 주석의 필터 바이트는 아래의 코드에서 +1과 관련이 있는지 *3과 관련이 있는지 모호함
    
    → 한 픽셀이 한 바이트인지, 200을 왜 추가하는지에 대한 명확한 이유를 알 수 없음
    
    → 코드만으로 설명이 되지 않아 주석을 달았는데, 주석 자체가 다시 설명을 요하는 상황이 됨
